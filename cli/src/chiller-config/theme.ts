...
/**
 * Create does not validate, assumes zod
 * has already validated and input is a
 * correctly formated ColorStyle
 */
export const create = (input: ColorStyle) => {
  const { style, primary } = (() => {
    if (input.startsWith('#')) return { style: `[${input}]`, primary: input }
    if (input.includes('-')) {
      const [color, level] = input.split('-')
      return { style: input, primary: TailwindColors[color][level] }
    }
    return {
      style: `${input}-400`,
      primary: TailwindColors[input]['400']
    }
  })()
  return {
